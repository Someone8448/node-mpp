const rl = new (require('readline').Interface)({input: process.stdin, output: process.stdout, prompt: ""})
const fs = require('fs')
...
if (cmd === "help") output = "Commands: test, help, channel, mute, volume, say"
if (cmd === "channel") client.setChannel(args)
if (cmd === "mute") {
    if (args.length == 0) return output = "Usage: mute [id] (boolean)"
    if (!['true','false'].includes(argsplit[1])) return output = "Invalid Boolean: true or false."
    if (argsplit[1] === "false") {output = `Unmuted ${argsplit[0]}`; if (db.data.mutes.includes(argsplit[0])) db.data.mutes.splice(db.data.mutes.indexOf(argsplit[0], 1))}
    if (argsplit[1] === "true") {output = `Muted ${argsplit[0]}`; db.data.mutes.push(argsplit[0])}
    db.write()
}
if (cmd === "volume") {
if (args.length == 0) return output = "Usage: volume [id] [percent]"
...
setTimeout(() => {
mapped[t].forEach(a => {
if (a.s) return midi.send('noteoff', {note: midikeys.indexOf(a.n) + 21, velocity: 0, channel: 1})
midi.send('noteon', {note: midikeys.indexOf(a.n) + 21, velocity: Math.floor(a.v * 128) * getVolume(n.p), channel: 1})
})
},Number(t))
})
...
n.n.forEach(a => {
 if (a.s) return setTimeout(() => midi.send('noteoff', {note: midikeys.indexOf(a.n) + 21, velocity: 0, channel: 1}),a.d)
        setTimeout(() => midi.send('noteon', {note: midikeys.indexOf(a.n) + 21, velocity: Math.floor(a.v * 128) * getVolume(n.p), channel: 1}),a.d)
...
setInterval(() => {
console.clear();
process.stdout.write(((message.length == 0) ? chalk.hex('#666666')('You can type here.') : chalk.hex('#008800')(message)) + `\n\nChannel: ${client.desiredChannelId}\n\n` + Object.values(client.ppl).map(p => `[` + (db.data.mutes.includes(p._id) ? chalk.hex('#ff0000')(p._id) : p._id) + `] ${chalk.bgHex(p.color)(p.name)} - ` + chalk.hex(convert.rgb.hex(Math.floor(getVolume(p._id) * -255) + 255, Math.floor(getVolume(p._id) * 255), 0))(`${nps.getNps(p._id)} NPS`)).join('\n') + "\n\n" + chat.join('\n') + "\n\n" + output + "\n")
},500)
var chat = []

client.on('a', msg => {
chat.push(`[${msg.p._id}] ${chalk.bgHex(msg.p.color)(msg.p.name)}: ${msg.a}`)
if (chat.length > 10) chat.splice(0,1)
})
client.on('c', c => {
chat = []
c.c.filter(a => a.m === "a").forEach(msg => {
chat.push(`[${msg.p._id}] ${chalk.bgHex(msg.p.color)(msg.p.name)}: ${msg.a}`)
if (chat.length > 10) chat.splice(0,1)
})
})
